...
function suite.test_list()
  print("Test: List scopes")

  local response = framework.request(
    "GET",
    framework.config.backoffice_url .. "/api/backoffice/scopes",
    {
...
function suite.test_create()
  print("Test: Create scope")

  local scope = {
    name = "payment-service" .. os.time(),
    description = "Payment service test scope created by integration test"
  }
  ...
  framework.assert.has_property(response.body, "id", "Created scope missing ID")
  framework.assert.equals(scope.name, response.body.name, "Name does not match")
  framework.assert.equals(scope.description, response.body.description, "Description does not match")

  test_data.scope = response.body

  return response.body.id ~= nil
end

function suite.test_get()
  print("Test: Get scope")

  if not test_data.scope.id then
    error("No scope ID to test")
  end

  local response = framework.request(
    "GET",
    framework.config.backoffice_url .. "/api/backoffice/scopes/" .. test_data.scope.id,
    {
      ["Authorization"] = "Bearer " .. test_data.admin_token,
      ["X-Trace-ID"] = framework.generate_trace_id(),
      ["X-Request-ID"] = framework.generate_request_id()
  ...
  framework.assert.status_code(response, 200)
  framework.assert.equals(test_data.scope.id, response.body.id, "IDs don't match")

  return true
end

function suite.test_update()
  print("Test: Update scope")
  ...
  local scope = {
    name = "payment-service-updated" .. os.time(),
    description = "Payment service updated test scope"
  }
  ...
  framework.assert.status_code(response, 200)
  framework.assert.equals(scope.name, response.body.name, "Name not updated")
  framework.assert.equals(scope.description, response.body.description, "Description not updated")

  return true
end

function suite.test_delete()
  print("Test: Delete scope")
